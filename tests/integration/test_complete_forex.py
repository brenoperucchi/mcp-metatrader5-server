#!/usr/bin/env python3
"""
[ROCKET] TESTE COMPLETO MCP MT5 FOREX - VALIDAÇÃO DEMO + TRADING REAL
====================================================================

Este script testa o servidor MCP MT5 configurado para Forex (Darwinex):
1. [OK] Verifica se a conta é DEMO 
2. [SECURE] Bloqueia trading em contas REAIS (segurança)
3. [TEST_TUBE] Testa TODAS as funcionalidades em conta demo Forex
4. [DATA] Executa operações reais no MetaTrader 5 com pares de moedas
5. [TRADE] Demonstra ciclo completo de trading no mercado Forex

[WARN]  ATENÇÃO: TRADING REAL APENAS EM CONTAS DEMO!
[FOREX] Servidor: http://192.168.0.125:50052 (Configuração Forex - Darwinex)
[FOREX] Conta: 3000064180 | Servidor: Darwinex-Demo
"""
import asyncio
import json
import sys
...
class MT5ForexTradingTest:
    ...
    async def test_connectivity(self):
        ...
        try:
            ...
            # Selecionar símbolo adequado para testes FOREX
            if symbol_list:
                print("[SEARCH] Procurando par de moedas adequado para testes...")
                
                # Símbolos preferenciais para teste Forex (principais pares)
                preferred_symbols = ["EURUSD", "GBPUSD", "USDJPY", "USDCHF", "AUDUSD", "USDCAD", "NZDUSD"]
                    ...
                    if pref_symbol in symbol_list:
                        ...
                        if not symbol_info.get("error"):
                            ...
                            if volume_min <= 1.0 and bid > 0 and ask > 0:
                                ...
                                print(f"[OK] Par de moedas preferencial encontrado: {pref_symbol} (vol_min: {volume_min}, bid: {bid}, ask: {ask})")
                                break
                
                # Se não encontrou símbolo preferencial, usar o primeiro forex disponível
                if not selected_symbol:
                    print("[WARN] Nenhum par preferencial encontrado, testando outros pares forex...")
                    for symbol in symbol_list[:30]:  # Testar apenas os primeiros 30
                        # Verificar se é um par de moedas (normalmente 6 caracteres)
                        if len(symbol) == 6 and any(currency in symbol for currency in ["USD", "EUR", "GBP", "JPY", "CHF", "AUD", "CAD", "NZD"]):
                            ...
                            if not symbol_info.get("error"):
                                ...
                                if volume_min <= 10.0 and bid > 0 and ask > 0:
                                    ...
                                    print(f"[OK] Par de moedas adequado encontrado: {symbol} (vol_min: {volume_min}, bid: {bid}, ask: {ask})")
                                    break
                ...
                print(f"[FOREX] Par de moedas selecionado para testes: {self.test_symbol}")
    ...
    async def test_account_validation(self):
        ...
        try:
            ...
            print("[GREEN] CONTA DEMO FOREX CONFIRMADA - Trading autorizado!")
            
            # Verificar se é realmente conta Darwinex
            server = account_data.get("server", "")
            if "Darwinex" in server:
                print(f"[FOREX] Servidor Darwinex confirmado: {server}")
            else:
                print(f"[WARN] Servidor não é Darwinex: {server}")
    ...
    async def test_market_data(self):
        ...
        try:
            ...
            if ticks.get("error"):
                ...
                await self.log_test("Dados Históricos", True, {"ticks": len(tick_data)})
    ...
    async def test_trading_history(self):
        """4.5. Teste de histórico de trading Forex"""
        try:
            print(f"[HISTORY] Testando funcionalidades de histórico Forex...")
            ...
            if history_orders.get("error"):
                ...
                print(f"[HISTORY] Buscando histórico Forex de {start_date_str} até {end_date_str}")
                ...
                if history_deals.get("error"):
                    ...
                    if deals_data:
                        ...
                        total_swap = sum(deal.get('swap', 0) for deal in deals_data)
                        total_volume = sum(deal.get('volume', 0) for deal in deals_data)
                        
                        print(f"[STATS] Lucro total: $ {total_profit:.2f}")
                        print(f"[STATS] Comissão total: $ {total_commission:.2f}")
                        print(f"[STATS] Swap total: $ {total_swap:.2f}")
                        print(f"[STATS] Volume total: {total_volume:.2f} lotes")
                        ...
                        for i, deal in enumerate(deals_data[-3:]):  # Últimos 3 deals
                            ...
                            print(f"   Deal {i+1}: {symbol} - Vol: {volume} @ {price:.5f} - P&L: $ {profit:.2f}")
                    else:
                        print("[INFO] Nenhum deal encontrado no histórico")
                
                # Filtrar histórico por par de moedas específico se disponível
                if self.test_symbol:
                    ...
                    if not symbol_deals.get("error"):
                        ...
                        if symbol_deals_data:
                            ...
                            symbol_volume = sum(deal.get('volume', 0) for deal in symbol_deals_data)
                            print(f"[SYMBOL] {self.test_symbol}: {len(symbol_deals_data)} deals, Vol: {symbol_volume:.2f}, P&L: $ {symbol_profit:.2f}")
                            
                            # [NOVO] Análise detalhada de uma ordem fechada específica - FOREX
                            if symbol_deals_data:
                                print(f"\n[DETAILED] Análise detalhada da ordem Forex fechada mais recente:")
                                latest_deal = symbol_deals_data[-1]  # Deal mais recente
                                
                                # Coletar TODOS os dados disponíveis do deal Forex
                                ...
                                print(f"[SYMBOL] Par de Moedas: {latest_deal.get('symbol', 'N/A')}")
                                print(f"[TYPE] Tipo: {latest_deal.get('type', 'N/A')}")
                                print(f"[ENTRY] Entry: {latest_deal.get('entry', 'N/A')}")
                                print(f"[VOLUME] Volume: {latest_deal.get('volume', 0):.2f} lotes")
                                print(f"[PRICE] Preço: {latest_deal.get('price', 0):.5f}")
                                print(f"[PROFIT] Lucro: $ {latest_deal.get('profit', 0):.2f}")
                                print(f"[COMMISSION] Comissão: $ {latest_deal.get('commission', 0):.2f}")
                                print(f"[SWAP] Swap: $ {latest_deal.get('swap', 0):.2f}")
                                print(f"[TIME] Hora: {latest_deal.get('time', 'N/A')}")
                                print(f"[COMMENT] Comentário: {latest_deal.get('comment', 'N/A')}")
                                
                                # Calcular spread e pips para análise Forex
                                profit = latest_deal.get('profit', 0)
                                volume = latest_deal.get('volume', 0)
                                profit_per_lot = 0
                                if volume > 0:
                                    profit_per_lot = profit / volume
                                    print(f"[FOREX] Lucro por lote: $ {profit_per_lot:.2f}")
                                
                                # Se temos o ticket da ordem, buscar dados completos da ordem
                                if order_ticket:
                                    print(f"\n[LOOKUP] Buscando dados completos da ordem Forex {order_ticket}...")
                                    ...
                                    if not order_details.get("error"):
                                        ...
                                        print(f"[ORDER_DETAIL] Preço de Setup: {order_data.get('price_open', 0):.5f}")
                                        print(f"[ORDER_DETAIL] SL: {order_data.get('sl', 0):.5f}")
                                        print(f"[ORDER_DETAIL] TP: {order_data.get('tp', 0):.5f}")
                                        print(f"[ORDER_DETAIL] Magic: {order_data.get('magic', 0)}")
                                        print(f"[ORDER_DETAIL] Tempo Setup: {order_data.get('time_setup', 'N/A')}")
                                        
                                        # Calcular pips para análise Forex
                                        setup_price = order_data.get('price_open', 0)
                                        execution_price = latest_deal.get('price', 0)
                                        if setup_price > 0 and execution_price > 0:
                                            price_diff = abs(execution_price - setup_price)
                                            # Assumir 4 ou 5 dígitos para cálculo de pips
                                            symbol = latest_deal.get('symbol', '')
                                            if 'JPY' in symbol:
                                                pips = price_diff * 100  # Pares com JPY (2 decimais)
                                            else:
                                                pips = price_diff * 10000  # Pares principais (4 decimais)
                                            print(f"[FOREX] Diferença de preço: {pips:.1f} pips")
                                        
                                        await self.log_test("Análise Detalhada de Ordem Forex", True, {
                                        ...
                                        await self.log_test("Análise Detalhada de Ordem Forex", False, 
    ...
    async def test_trading_operations(self):
        """5. Teste de operações de trading REAIS no mercado Forex"""
        if not self.test_symbol:
            await self.log_test("Trading", False, error="Nenhum símbolo para trading")
            return False
        
        try:
            print("\n[ALERT] INICIANDO TESTES DE TRADING REAL EM CONTA DEMO FOREX!")
            print("=" * 70)
            
            # Obter informações do símbolo para volume mínimo
            print(f"[SEARCH] Obtendo informações do par {self.test_symbol}...")
            symbol_info = await self.client.get_symbol_info(self.test_symbol)
            if symbol_info.get("error"):
            ...
            print(f"[DATA] Step do volume: {volume_step}")
            
            # Para Forex, usar volume mínimo padrão (0.01 lote = micro lote)
            test_volume = max(volume_min, 0.01)
            
            print(f"[FOREX] Volume para testes: {test_volume} lotes")
            
            # Verificar se símbolo está ativo e negociável
            if not symbol_data.get("visible", True) or not symbol_data.get("select", True):
                print(f"[WARN] Par {self.test_symbol} pode não estar ativo para trading")
            ...
            buy_result = await self.client.order_send(
                ...
                comment="Teste MCP Demo Forex - Compra"
            )
            ...
            if retcode == 10009 and buy_ticket:
                ...
                if not positions.get("error"):
                    ...
                    if pos_list:
                        ...
                        print("[DATA] DETALHES INICIAIS DA POSIÇÃO FOREX:")
                        print("=" * 60)
                        for key, value in position.items():
                            if isinstance(value, float):
                                if key in ["price_open", "price_current", "sl", "tp"]:
                                    print(f"   {key}: {value:.5f}")  # Mais decimais para forex
                                else:
                        ...
                        # Calcular SL e TP baseados no preço de entrada (para Forex)
                        entry_price = position.get("price_open", 0)
                        point = symbol_data.get("point", 0.00001)  # Ponto para forex
                        
                        # Para forex: SL 50 pips abaixo, TP 100 pips acima
                        stop_loss = entry_price - (50 * point)   # 50 pips abaixo
                        take_profit = entry_price + (100 * point)  # 100 pips acima
                        
                        print(f"   [MONEY] Preço de entrada: {entry_price:.5f}")
                        print(f"   [STOP] Stop Loss: {stop_loss:.5f} (50 pips)")
                        print(f"   [TP] Take Profit: {take_profit:.5f} (100 pips)")
                        ...
                        if not updated_positions.get("error"):
                            ...
                            if updated_pos_list:
                                ...
                                if updated_position:
                                    ...
                                    print("[WARN] Posição não encontrada após modificação")
                            else:
                        ...
                        # 5.2.3 Fechar a posição usando o position ticket correto
            ...
            # Ordem 20 pips abaixo do preço atual
            limit_price = current_bid - (20 * point)
            ...
            limit_result = await self.client.order_send_limit(
                ...
                price=limit_price,
                comment="Teste MCP Demo Forex - Limit"
            )
            ...
            if limit_result.get("error"):
                ...
                if retcode == 10009 and limit_ticket:
                    ...
                    new_price = current_bid - (30 * point)  # 30 pips abaixo
            ...
            print("=" * 70)
            print("[SUCCESS] TODOS OS TESTES DE TRADING FOREX CONCLUÍDOS!")
            return True
    ...
    async def run_complete_test(self):
        ...
        print("[ROCKET] INICIANDO TESTE COMPLETO MCP MT5 FOREX")
        print("=" * 60)
        ...
        try:
            ...
            # Relatório final
            print("\n[DATA] RELATÓRIO FINAL DOS TESTES FOREX")
            print("=" * 50)
            ...
            # Salvar relatório
            with open("test_report_forex.json", "w") as f:
                json.dump(self.test_results, f, indent=2)
            print("\n[SAVE] Relatório salvo em: test_report_forex.json")
...
async def main():
    """Função principal"""
    print("*** MCP METATRADER 5 FOREX - TESTE COMPLETO DE TRADING ***")
    print("Testando funcionalidades completas com validação de segurança - Mercado Forex")
    print("=" * 80)
    ...
    print(f"[NET] Conectando ao servidor Forex: {server_url}")
    print("[FOREX] Conta esperada: 3000064180 | Servidor: Darwinex-Demo")
    
    # Executar testes
    tester = MT5ForexTradingTest(server_url)
    success = await tester.run_complete_test()
    
    if success:
        print("\n[SUCCESS] TODOS OS TESTES FOREX PASSARAM!")
        print("[OK] Sistema MT5 MCP Forex funcionando perfeitamente")
        sys.exit(0)
    else:
        print("\n[FAIL] ALGUNS TESTES FOREX FALHARAM!")
